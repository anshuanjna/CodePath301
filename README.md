...
- [ ] Test case 3: updateFileValues converts dateAdded into a numeric epoch timestamp (not a string), and two files uploaded in the same minute produce equal dateAdded values (confirms .getTime() correctly identifies real ties without introducing false ones — files-page/data/utils.test.js).

### Integration Tests

- [ ] Integration scenario 1 - Same-minute tied files, rendered via the real FilesPage component and a real Sort & Filter modal interaction, produce a consistent, direction-respecting order across repeated Newest/Oldest toggles (FilesPage.sort.test.jsx).
- [ ] Integration scenario 2 - Files spanning different weekdays, rendered via the real FilesPage component, sort in genuinely correct chronological order in both directions (FilesPage.sort.test.jsx).
